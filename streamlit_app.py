...
if not gemini_api_key:
    ...
    """
        ...
        2. 문맥을 고려했을 때 개인의 정보를 유추할 수 있는 특정 단서가 개인정보 요소와 관련있는지 확인하세요.
        3. 개인과 관련되거나 상호작용한 모든 요소들은 잠재적인 개인정보 요소에 해당합니다.
        4. 간접적으로 알 수 있는 개인정보도 약간의 가능성이 있다면 모두 탐지해주세요.
        5. 답변 시 입력된 게시물에서 개인정보 요소는 해당 내용에 맞는 [ ] 태그로 표시해 주세요.
        6. 탐지 예시의 스타일과 동일하게 답변을 해주세요. 답변에는 입력, 태그, 결과가 모두 포함되어야 합니다.
        ...
        5. 정신적 정보
            5.1 기호 및 성향 정보: 도서.비디오 등 대여기록, 잡지구독정보, 물품구매내역, 웹사이트검색내역 등
            5.2 내면의 비밀 정보: 사상, 신조, 종교, 가치관, 정당 노조 가입여부 및 활동내역, 개인이 제작한 컨텐츠(글, 영상, 사진) 등
        6. 기타 정보
            6.1 통신정보: E-mail 주소, 전화통화내역, 로그파일, 쿠키, SNS 주소, 커뮤니티 활동 내역 등
    ...
    """
        ...
        5. 답변 시 입력된 게시물에서 개인정보 요소는 해당 내용에 맞는 [ ] 태그로 표시해 주세요.
        6. 탐지 예시의 스타일과 동일하게 답변을 해주세요. 답변에는 입력, 태그, 결과가 모두 포함되어야 합니다.
        7. 입력된 답변을 분석한 뒤 마지막에 최종 답변을 출력하시오.
        8. 결과는 인적사항, 신체적정보, 사회적정보, 재산적정보, 정신적 정보, 기타 정보에 대해서만 집계하세요.
    ...
    if user_input:
        if st.button("분석 시작"):
            ...
            input_second = f"""
                ...
                ## 차근차근 분석해보자
                입력된 문장의 개별 정보를 모두 표시하고 연결되는 정보를 결합(n-gram)해가며 계층적으로 분석한 뒤 쌍을 제시하시오.
                특정된 인물을 표시하고 앞서 제시된 쌍 및 정보와 함께 해당 인물과 직접적으로 혹은 간접적으로 연관된 요소는 무엇이 있는지 태그를 달아 제시하시오.
                제시된 쌍과 요소에서 유추할 수 있는 정보를 분석하고 알맞은 태그를 달아 제시하시오.
                앞서 제시된 요소를 고려하여 답변을 수정하시오.
